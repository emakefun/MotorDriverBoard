...
#### 2、只通过接线柱供单一电源给驱动板，Uno主板和舵机供电。将驱动板的5V电源输出到Uno主板上，此时接线柱的电压范围为6~36V。
...
1. **Mixly云端安装(只适合mixly2.0)**
   ...
   ![mixly_guanliku](./doc/picture/ZH/mixly/mixly_guanli.png)

   第二步

   ![mixly_kuanzhuang](./doc/picture/ZH/mixly/mixly_kuanzhuang.png)

   第三步

   ![install_success](./doc/picture/ZH/mixly/install_success.png)

   第四步 当看到一下界面，就证明安装成功。

   ![mixly_success](./doc/picture/ZH/mixly/mixly_success.png)
...
2. **本地安装**
   ...
   ![local_install](./doc/picture/ZH/mixly/local_install.png)

   步骤三

   ![select_xml](./doc/picture/ZH/mixly/select_xml.png)
...
##### A ：如果用Uno的DC头供电，最大能够驱动12V 3A的电机，如果是驱动板上的接线柱供电，可以最大驱动16V 3A的电机。我们供电必须要先知道电机的额定电压是多少，比如额点电压是6V-1A，那么我们供电就只能是8.4V锂电池供电（如果用用12V供电有概率会烧电机），如果额定电压是12V的电机，那么供电也要用三节锂电池11.1V的电池。总之电机额定电压和电流不要和实际供电电源相差很大，否则驱动不了电机。
...
##### A ：在使用驱动板的时候，如果是新手用户，请一定要一步步来熟悉使用，不要一上来就全部功能一起使用，然后问的问题很大。针对这个问题需要做如下测试才行

a、驱动板是否使用7V以上锂电池供电且正常，板子上Power灯是否亮起
b、下载直流电机驱动测试程序**确保要上传成功**，电机接线正确，来确定板子驱动正常
c、直接把锂电池的电压加到直流电机上，来证明电机是好的。
d、通过上传PS2接收测试程序，来证明PS2遥控器是好的
e、前面正确后下载PS2控制四驱小车程序，并按按键控制电机
